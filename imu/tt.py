...
ax.set_xlim([-0.15, 0.15])
ax.set_ylim([-0.15, 0.15])
# --- MODIFICATION: Set Z-axis limits to be symmetrical around 0 ---
ax.set_zlim([-0.15, 0.15]) # Changed from [0, 0.15]
# --- End Modification ---
ax.legend()
ax.grid(True)

# --- MODIFICATION: Add lines for X, Y, Z axes ---
# Get the axis limits
xlim = ax.get_xlim()
ylim = ax.get_ylim()
zlim = ax.get_zlim()
# X-axis (red)
ax.plot([xlim[0], xlim[1]], [0, 0], [0, 0], 'r-')
# Y-axis (green)
ax.plot([0, 0], [ylim[0], ylim[1]], [0, 0], 'g-')
# Z-axis (blue)
ax.plot([0, 0], [0, 0], [zlim[0], zlim[1]], 'b-')
# --- End Modification ---
